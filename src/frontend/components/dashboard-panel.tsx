...
export function DashboardPanel({ data, onOpenCategory }: { data: RulesData; onOpenCategory: (category: RuleCategory) => void }) {
  ...
  return <div className="page-stack dashboard-page">
    ...
    <section className="soft-card unified-card recent-section">
      ...
      <div className="recent-list sort-content-transition" key={`${sortKey}-${sortDirection}`}>{recentCategories.map((category) => <button className="recent-row" key={category.id} onClick={() => onOpenCategory(category)}><CategoryIcon icon={category.icon} name={category.name}/><span className="recent-copy"><strong data-no-translate>{category.name}</strong><small data-no-translate={Boolean(category.description)}>{category.description || '暂无分类说明'}</small></span><span className="recent-meta"><strong>{category.rules.length}</strong><small>条规则</small></span><time dateTime={category.updatedAt}>{new Date(category.updatedAt).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</time><UiIcon name="chevronRight" size={20}/></button>)}{!recentCategories.length && <div className="empty-state"><UiIcon name="rules" size={30}/><strong>还没有规则分类</strong><span>前往规则页创建第一个分类</span></div>}</div>
    </section>
  </div>;
}
